...
func TestWebSocketProxy(t *testing.T) {
	...
	_ = websocket.Message.Receive(ws, &response)
	if err != nil {
		t.Fatalf("err %s", err)
	}
...
func TestBasicAuthPassword(t *testing.T) {
	...
	cookie := rw.Result().Header["Set-Cookie"][1]
...
func (pat_test *PassAccessTokenTest) getCallbackEndpoint() (http_code int,
	...
	return rw.Code, rw.Result().Header["Set-Cookie"][1]
}
...
func TestLoadCookiedSession(t *testing.T) {
	...
	_ = pc_test.SaveSession(startSession, time.Now())
...
func TestProcessCookieRefreshNotSet(t *testing.T) {
	...
	startSession := &providers.SessionState{Email: "michael.bland@gsa.gov", AccessToken: "my_access_token"}
	_ = pc_test.SaveSession(startSession, reference)
...
func TestAuthOnlyEndpointAccepted(t *testing.T) {
	...
	_ = test.SaveSession(startSession, time.Now())
...
func TestAuthOnlyEndpointUnauthorizedOnExpiration(t *testing.T) {
	...
	startSession := &providers.SessionState{
		Email: "michael.bland@gsa.gov", AccessToken: "my_access_token"}
	_ = test.SaveSession(startSession, reference)
...
func TestAuthOnlyEndpointSetXAuthRequestHeaders(t *testing.T) {
	...
	assert.Equal(t, "oauth_user", pc_test.rw.Result().Header["X-Auth-Request-User"][0])
	assert.Equal(t, "oauth_user@example.com", pc_test.rw.Result().Header["X-Auth-Request-Email"][0])
}
...
func (v *SignatureAuthenticator) Authenticate(w http.ResponseWriter, r *http.Request) {
	...
	if result == hmacauth.ResultNoSignature {
		_, _ = w.Write([]byte("no signature received"))
	} else if result == hmacauth.ResultMatch {
		_, _ = w.Write([]byte("signatures match"))
	} else if result == hmacauth.ResultMismatch {
		_, _ = w.Write([]byte("signatures do not match:" +
			"\n  received: " + headerSig +
...
func NewSignatureTest() *SignatureTest {
	...
	providerHandler := func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"access_token": "my_auth_token"}`))
	}
