...
func TestOAuthProxyE2E(t *testing.T) {
	...
	require.NoError(t, err)
	userClient, _ := userclient.NewForConfig(testConfig)
	ns := CreateTestProject(t, kubeClient, projectClient)
	defer func() {
		if len(os.Getenv("DEBUG_TEST")) > 0 {
			return
		}
		_ = kubeClient.CoreV1().Namespaces().Delete(testCtx, ns, metav1.DeleteOptions{})
	}()
	...
	_ = WaitForClusterOperatorStatus(t, configClient.ConfigV1(), nil, pbool(true), nil)
	_ = WaitForClusterOperatorStatus(t, configClient.ConfigV1(), pbool(true), pbool(false), nil)
	...
	for tcName, tc := range oauthProxyTests {
		...
		t.Run(fmt.Sprintf("setting up e2e tests %s", tcName), func(t *testing.T) {
			...
			defer func() {
				...
				_ = kubeClient.CoreV1().Pods(ns).Delete(testCtx, "proxy", metav1.DeleteOptions{})
				_ = kubeClient.CoreV1().Services(ns).Delete(testCtx, "proxy", metav1.DeleteOptions{})
				_ = deleteTestRoute("proxy-route", ns)
				_ = kubeClient.CoreV1().ConfigMaps(ns).Delete(testCtx, "proxy-certs", metav1.DeleteOptions{})
				_ = kubeClient.CoreV1().ServiceAccounts(ns).Delete(testCtx, "proxy", metav1.DeleteOptions{})
				_ = waitForPodDeletion(kubeClient, oauthProxyPod.Name, ns)
				_, _ = execCmd("oc", []string{"adm", "policy", "remove-role-from-user", "admin", user, "-n", ns}, "")
			}()
